...
export const getGameDetailsById = async (
    ...
    const gameDetailsUrl = `${managementApiUrl}/games/${gameId}`

    console.log('Getting game details from Management API:', gameDetailsUrl)
    ...
    if (!response.ok) {
        ...
        console.error('Management API game details error:', {
        ...
        throw new Error(
            `Error getting game details from Management API: ${response.status} ${errorBody}`,
        )
    }
